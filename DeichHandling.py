...
class DeichHandler:
    ...
    def DeichPutDown(self, checkPoint, dislocated = 1):
        ...
        self.Gripper2.movemotor(100, True)
        self.DriveTrain.driveForward(RobotContainer.SLOW_SPEED, -3)
        ...
        if color in humans:
            if dislocated == 0:
                ...
                self.DriveTrain.driveForward(RobotContainer.SLOW_SPEED, -3)
                self.DriveTrain.turnAngle(RobotContainer.TURN_SPEED, angle)
                self.DriveTrain.driveForward(RobotContainer.SPEED, 20)
                self.DriveTrain.followLine(RobotContainer.SPEED, RobotContainer.AGGRESSION, RobotContainer.LINE, 33)
                self.DriveTrain.turnAngle(RobotContainer.TURN_SPEED, angle)
                self.DriveTrain.driveForward(RobotContainer.SLOW_SPEED, -19)
                self.Gripper2.movemotor(50, False)
                sleep(0.5)
                self.DriveTrain.driveForward(RobotContainer.SPEED, 2)
                self.DriveTrain.center("Black", direction='-1')
                self.DriveTrain.followLine(RobotContainer.SPEED, RobotContainer.AGGRESSION, RobotContainer.LINE, 29)
